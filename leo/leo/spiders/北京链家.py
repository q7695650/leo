...
class bjlianjia(scrapy.Spider):
    ...
    def parse(self, response):
        sections=response.xpath('//div[@data-role="ershoufang"]//a/@href').extract()
        for section in sections:
            if not section.startswith('https'):
                section='https://bj.lianjia.com'+section
                yield scrapy.Request(url=section,callback=self.parse_street)

    def parse_street(self,response):
        streets=response.xpath('//div[@data-role="ershoufang"]/div[2]/a/@href').extract()
        for street in streets:
            street='https://bj.lianjia.com'+street
            yield scrapy.Request(url=street,callback=self.parse_money)
